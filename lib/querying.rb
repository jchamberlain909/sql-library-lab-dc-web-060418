def select_books_titles_and_years_in_first_series_order_by_year
  <<-SQL
    ...
    FROM books 
    INNER JOIN series ON books.series_id=series.id WHERE series.id = (SELECT MIN(id) from series)
  SQL
end
...
def select_character_names_and_number_of_books_they_are_in
  ...
  FROM characters INNER JOIN character_books ON character_books.character_id = characters.id INNER JOIN books ON books.id = character_books.book_id
